namespace 'cuke_slicer' do
  ...
  task :rubocop do
    ...
    completed_process = CukeSlicer::CukeSlicerHelper.run_command(['bundle', 'exec', 'rubocop',
                                                                  '--format', 'fuubar',
                                                                  '--format', 'html', '--out', "#{ENV.fetch('CUKE_SLICER_REPORT_FOLDER')}/rubocop.html", # rubocop:disable Metrics/LineLength
                                                                  '-S', '-D'])
  ...
  task :check_dependencies do
    ...
    completed_process = CukeSlicer::CukeSlicerHelper.run_command(['bundle', 'outdated',
                                                                  'cuke_modeler',
                                                                  '--filter-major'])
  ...
  task :full_check do
    ...
    Rake::Task['cuke_slicer:rubocop'].invoke
