...
RSpec.describe 'the gem' do
  ...
  it 'is named correctly' do
    expect(@gemspec.name).to eq('cuke_slicer')
  end
  ...
  it 'has a version' do
    expect(@gemspec.version.version).to eq(CukeSlicer::VERSION)
  end
  ...
  it 'has a homepage' do
    expect(@gemspec.homepage).to eq('https://github.com/enkessler/cuke_slicer')
  end

  it 'has a summary' do
    text = <<-TEXT
      A gem for extracting test cases from a Cucumber test suite.
    TEXT
  ...
  it 'has a description' do
    text = <<-TEXT
      Slices a Cucumber test suite into the smallest possible executable pieces (i.e. scenarios
  ...
  describe 'metadata' do
    ...
    it 'links to the changelog' do
      expect(@gemspec.metadata['changelog_uri']).to eq('https://github.com/enkessler/cuke_slicer/blob/master/CHANGELOG.md')
    end

    it 'links to the known issues/bugs' do
      expect(@gemspec.metadata['bug_tracker_uri']).to eq('https://github.com/enkessler/cuke_slicer/issues')
    end

    it 'links to the source code' do
      expect(@gemspec.metadata['source_code_uri']).to eq('https://github.com/enkessler/cuke_slicer')
    end

    it 'links to the gem documentation' do
      expect(@gemspec.metadata['documentation_uri']).to eq('https://www.rubydoc.info/gems/cuke_slicer')
    end
  ...
  describe 'included files' do
    ...
    it 'includes the gemspec file' do
      gemspec_file = 'cuke_slicer.gemspec'
  ...
  describe 'dependencies' do
    ...
    it 'works with CukeModeler 3' do
      ...
      expect(cuke_modeler_version_limits).to match_array(['>=3.2', '<4.0'])
    end
...
RSpec.describe CukeSlicer do

  it 'has a version number' do
    expect(CukeSlicer::VERSION).not_to be nil
  end
